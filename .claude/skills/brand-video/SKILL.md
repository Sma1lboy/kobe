...
**每支视频是一个独立项目 `videos/<name>/`**(自带 package.json / tsconfig / .gitignore,
...
"青绿 hacker 风清零"式返工的结构性根治。worked example:[`videos/kobe-intro/`](../../../videos/kobe-intro/)。

## Step 0 — storyboard(和 ref-replay 同一纪律,最高杠杆)

输入:口播稿或 `transcription.srt`(可选配好的 `voiceover.mp3`)。
通读后写 `videos/<name>/storyboard.md`:
