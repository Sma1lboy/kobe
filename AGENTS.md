...
- **Tech stack is locked:** TypeScript + `@opentui/core` + `@opentui/react` + React 19 + Bun. Do not re-litigate. (The Solid TUI was removed 2026-07-07 — React is the only UI. `solid-js` remains as a dependency ONLY for its reactive-signals primitives in the orchestrator/client core, not for any UI.)
