...
**Current state**: the Solid `testRender` harness (`test/render/harness.tsx`) and every component-mount test it drove were removed with the Solid TUI. The track today holds only the framework-free PTY-host tests (`pty-host.test.ts`, `pty-hosted.test.ts`). Rebuilding a React component-mount harness on `@opentui/react`'s test-utils — so dialogs/panes get frame-level coverage again — is a tracked follow-up.
