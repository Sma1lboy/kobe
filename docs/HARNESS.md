...
- **`bun run perf:golden` is the release-ritual doctor** (`scripts/perf-golden.ts`): five end-to-end metrics against a throwaway sandbox pty-host — CLI cold start, PTY spawn→first output, park→wake replay latency, hot-tab RSS cost, park heap reclaim — each with a committed golden ceiling set 2-3× the reference-machine numbers, so it catches structural regressions, not runner jitter. Local + pre-release only; it stays out of CI for the same wall-clock reason as `bench`.
