...
Compared against [herdr.dev/docs](https://herdr.dev/docs/) (`refs/herdr/docs/next/website/src/content/docs/*.mdx`, English set). Purpose: map every herdr documented feature/doc to kobe: what we already have (→ document it), what we lack (→ borrow or explicitly skip).

## Doc-surface mapping

| Herdr doc | kobe status | Action |
|---|---|---|
...
| agents (supported agents, detection, labels, attach) | Engines claude/codex/copilot/kimi + custom engines; no user doc; no `kobe attach` cmd | **Wrote `docs/ENGINES.md`**; borrow: direct-attach command (gap below) |
| agent-skill | `.agents/skills/kobe/SKILL.md` + `kobe skill install` | OK (stronger than herdr's) |
| agent-automation | Engine hooks + plugin events + `kobe api` | Covered by `docs/CLI.md` + PLUGIN-AUTHORING |
...
| socket-api | `docs/design/cli-api.md` is historical; `kobe api schema` is source of truth | **Wrote `docs/CLI.md`** (CLI + API reference, schema-first) |
...
| marketplace | GitHub topic `kobe-plugin` + `kobe plugin search` + landing page | OK (same zero-infra model, independently built) |
...
Both are already broad and were under-documented rather than missing: full-mouse TUI (click-focus everywhere, wheel with native emulator semantics, drag selection, shift-drag bypass) and a three-signal notification surface (OSC 9 desktop notifications that ride SSH, bundled `pulse.wav` + BEL fallback, 4.5 s toasts, unread dots, cross-task attention inbox via `prefix+i` / `F7`, `kobe api notify` for scripts). Covered in `docs/TROUBLESHOOTING.md`, `docs/CONFIGURATION.md`, and `docs/KEYBINDINGS.md`.

## Deliberate skips

- **Named sessions / workspaces-as-namespaces**: kobe's task list is already the namespace; herdr's three-level workspace/tab/pane model solves a problem kobe doesn't have.
- **Mouse-first pane management, drag borders, right-click menus**: herdr is a multiplexer; kobe's layout is fixed flex panes. Mouse support exists for click/scroll; multiplexer-style pane surgery is out of scope.
- **Prefix-mode copy mode with tmux motions**: kobe relies on the embedded terminal's own scrollback/selection; revisit only if users ask.
...
- `docs/CLI.md`: consolidated CLI + `kobe api` reference (points at `kobe api schema` as source of truth)
