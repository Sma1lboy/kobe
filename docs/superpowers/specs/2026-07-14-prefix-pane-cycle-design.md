...
override contract. Add a global, prefix-only `focus.previous` binding for `j`.
Reuse the existing global `focus.next` binding for `prefix+k` while preserving
its direct `F4` chord.

The workspace host maps `focus.previous` to the existing pane-cycle helper with a
...
User keybinding files can override `focus.previous` and `focus.next` through the
...
- previous/next action routing through the real key-dispatch registration;
- user overrides for `focus.previous` and `focus.next`;
