...
Task title source becomes explicit:

- `placeholder`: initial `(new task)`.
- `fallback`: deterministic first-prompt fallback.
- `ai`: generated by the engine title generator.
- `manual`: set by user rename or API rename.

Auto-title may update tasks whose source is `placeholder`, `fallback`, or `ai`. It must not update `manual`.
...
7. If the AI title succeeds and the task has not been manually renamed, replace the fallback with the AI title.
...
Manual rename through `r` or `kobe api rename` sets the title source to `manual`.
