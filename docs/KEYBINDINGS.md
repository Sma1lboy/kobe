...
| tmux `prefix space` | tmux prefix | Toggle Zen mode — collapse the ChatTab to the engine pane (hides the file/Ops and terminal panes, and the Tasks rail unless Settings → General → "Keep Tasks pane in zen mode" is on). A second press restores exactly the panes zen hid. Also reachable via the `zen` chip above the file list (enter-only, since entering hides that pane). |
