...
> **PureTUI prefix (2026-07-11).** ChatPane and cross-pane control rows use the configurable prefix sequence `ctrl+a`, then the action's second key (for example `ctrl+t` becomes `ctrl+a t`); Tasks, Files, and non-conflicting Terminal-local rows retain their direct Ctrl chords. A row may also expose both forms as deliberate aliases, but there is no focus-time fallback or priority rule. Five rows went back to direct-only (owner call 2026-07-11): `chat.tab.new`/`close`/`cycle-next`/`cycle-prev` fire on `ctrl+t`/`ctrl+w`/`ctrl+]`/`ctrl+[` (tab switching is a repeated action where a two-stroke prefix per hop is unusable) and `focus.sidebar` fires on `ctrl+q` (THE escape hatch is too load-bearing for two strokes). `focus.numeric` keeps both forms (#310): `ctrl+h/j/k/l` direct plus the `prefix h/j/k/l` aliases. `chat.tab.chooseEngine` went back to direct-only `ctrl+e` (owner call 2026-07-12). The remaining moved rows (`ctrl+f`/`ctrl+\`/`ctrl+=`) stay prefix-only. tmux Handover bindings remain documented as tmux behavior below and are unaffected.
...
`ctrl+q` from any non-sidebar pane jumps back to the sidebar (`focus.sidebar`). In Tasks, a second
...
| `b` / `d` (files) | `files.scope` (working ↔ vs-base) / `files.diff` (open read-only diff) vs sidebar `d` (archive), composer typing | Both are `scope: "files"` plain letters, registered only while the Files pane is focused — the pane-scope rule keeps them off the sidebar's `d` and out of composer text. `d` opens the current file's read-only diff in a workspace content tab (a content swap, no focus pull — KOB-25); `b` toggles the Changes tab's git scope. `files.diff` is deliberately distinct from `files.open` (`enter`, editable nvim): `enter` edits + pulls focus, `d` previews read-only + keeps focus. |
