...
| `ctrl+g`         | readline bell / abort-ish muscle memory in some shells | Workspace-scoped "toggle Claude Agent View." The chord only fires while the workspace owns focus and Agent View is available for the active Claude tab, so it does not affect sidebar/files/terminal panes or non-Claude tabs. |
