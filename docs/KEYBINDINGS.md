...
| ---------------- | --------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `ctrl+1..4`      | `focus.numeric` (global) wins everywhere | **Pane focus has hard precedence.** Chat tab pick was originally `ctrl+1..9` and shadowed pane focus in workspace + multi-tab. Numeric pick removed entirely — chat tabs cycle via `ctrl+]` / `ctrl+[` (mirroring sidebar's `[/]` view switch and the files pane's `[/]` tab cycler). `ctrl+N` is now uncontested as the global pane-focus chord. **Also registers `alt+N`** as an always-works fallback — most terminals (Terminal.app, iTerm default, tmux without extended keys) don't propagate ctrl+digit; alt+digit produces `ESC<digit>` which every terminal parses, so the chord works regardless of kitty / CSI-u support. |
