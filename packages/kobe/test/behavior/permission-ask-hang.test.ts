...
import { applyEvent, createInitialState } from "../../src/tui/panes/chat/store"
...
describe("KOB-4: Permission Request Event Handling", () => {
  ...
  it.skip("hypothetical permission_request EngineEvent would be silently dropped by chat reducer", async () => {
    ...
    // The shape is conjectural — this test documents the gap, not the
    // wire format. Casting through `unknown` rather than `any` keeps
    // Biome's no-explicit-any lint clean.
    ...
    const nextState = applyEvent(state, hypotheticalEvent)
  ...
  it.skip("parser silently drops unknown top-level stream-json types (including hypothetical permission_request)", () => {
  ...
  it.skip("SessionPump.run would not pause for a hypothetical permission_request", async () => {
