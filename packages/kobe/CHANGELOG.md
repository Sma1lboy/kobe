...
- **The branch follows the auto-name too** — when a task picks up its name from the first prompt, its still-default `kobe/new-task-…` branch is renamed in lockstep (real `git branch -m`). It only fires while untouched: a branch you renamed yourself is left alone. Branch slugs stay ASCII, so a non-Latin title (e.g. Chinese) falls back to `kobe/task-…`.
- **Every ChatTab names itself from its own first prompt** — each ChatTab window stops reading `claude`/`zsh` and is renamed to the first message you send in it, so a task with several Ctrl+T tabs shows what each one is about. Claude tabs are mapped exactly: kobe launches claude with a known `--session-id` and reads back that tab's own transcript. Codex (which can't take a caller-set session id) names only its first tab. A tab you renamed with F2 is never overwritten.
