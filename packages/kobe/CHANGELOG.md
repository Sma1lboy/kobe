...
- **The outer monitor's live preview now fills its box** — the WORKSPACE preview captures only the inner claude pane, which is ~47% of the tmux window, so it used to leave the wide preview box half-empty. The preview now measures its own box and (only while no client is attached) `resize-window`s the inner session so the claude pane is about as wide as the box before capturing. `window-size` stays `latest`, so attaching (⏎) snaps the window back to your real terminal — the preview resize never fights a live session, and the inner four-pane layout is untouched (KOB-244).
