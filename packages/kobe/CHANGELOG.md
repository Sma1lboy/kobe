...
- **GitHub Copilot CLI can be selected as a local engine** — adds a first-class `copilot` adapter alongside Claude Code, Codex, and Gemini, using Copilot prompt-mode JSONL, Copilot model/effort choices, local session history under `~/.copilot`, and plan/full-access permission mapping (KOB-164).
