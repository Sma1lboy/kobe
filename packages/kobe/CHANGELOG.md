...
- **Claude chat tabs can switch into Agent View** — Claude-backed workspace tabs now expose a Chat/Agents submode, normalize Claude Code background agents from the engine-owned `~/.claude/sessions` index, and list matching background sessions for the active worktree (KOB-140).
- **Claude Agent View can start background agents** — the Agents submode now includes a prompt box that launches Claude Code's own `claude --bg` flow through the engine adapter, then refreshes the normalized background-agent list for the worktree (KOB-141).
- **Claude Agent View background rows open their chat session** — clicking a listed background agent now activates or creates the matching Claude chat tab and returns the workspace to Chat so the transcript is visible (KOB-143).
- **Claude Agent View covers all background states** — background rows now distinguish Working, Needs input, Idle, Completed, Failed, and Stopped with the intended status grouping and colors (KOB-144).
...
- **Agent View rows match Claude's summary shape** — background rows now show session name, meaningful activity when available, and last-changed age without repeating grouped status or exposing per-row session ids / worktree paths (KOB-147).
- **Opened background history refresh preserves pending-input controls** — background-session transcript refresh now pauses once a tab is awaiting input, so `AskUserQuestion` selection state is not reset while the user is choosing an answer (KOB-146).
- **Pending input rows are not rebuilt by history hydration** — chat history hydration now leaves an already-rendered pending approval/question row in place, preserving local picker selection state while background history replay catches up (KOB-146).
- **Background history polling is bg-only** — Agent View marks opened background sessions explicitly, and the extra transcript refresh / pending-input rehydrate path now runs only for those tabs so normal Chat `AskUserQuestion` behavior stays on the mainline path (KOB-146).
- **Answered background questions stay answered** — history-derived `AskUserQuestion` requests are tombstoned after the user responds so later transcript refreshes cannot resurrect the same pending picker while Claude is still writing the tool result (KOB-146).
