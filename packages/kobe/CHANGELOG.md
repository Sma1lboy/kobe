...
- **Background tasks ring the terminal bell and flash the sidebar row when a turn finishes** — while you're working in task A, task B / C / D quietly streaming in the background now signal completion the moment they're done: a `\x07` on stderr (your terminal decides whether that's audible, a visual bell, or a dock-badge bump) plus a ~1.2s background flash on the matching sidebar row (`theme.success` for clean done, `theme.error` on engine error). Switching to the task before the timer expires dismisses the flash; no manual ack needed (KOB-153).
