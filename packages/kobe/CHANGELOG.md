...
- **The outer monitor's live preview is no longer cramped to ~half width** — the per-task tmux layout dropped the experimental far-left Tasks pane and widened the engine pane (claude/codex) to 72% of the window, so `capture-pane` returns wide content that fills the workspace preview instead of a narrow column. Switch between tasks from the outer monitor; the in-session Tasks pane and its `<prefix> f` quick-create chord are no longer in the default layout (the `kobe tasks` / `kobe quick-create` machinery is kept for revival) (KOB-244).
