...
export class RemoteOrchestrator {
  ...
  async listBackgroundAgents(taskId: string): Promise<BackgroundAgent[]> {
    const res = await this.client.request<{ agents: BackgroundAgent[] }>("agent.background.list", { taskId })
    return res.agents
  }
