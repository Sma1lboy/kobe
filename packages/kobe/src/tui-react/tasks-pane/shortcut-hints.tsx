...
/**
 * The Tasks pane's bottom shortcut legend — React port of
 * `src/tui/tasks-pane/shortcut-hints.tsx` (React is the default runtime
 * since 2026-07-07). Same behavior: a self-contained component with only
 * plain-value props in (no writes back into `TasksShell`'s state). Solid→
 * React deltas: `Accessor` props become plain values, `createSignal`→
 * `useState`, `onMount`→`useEffect(…, [])`, the `keymapVersion()` reactive
...
export function ShortcutHints(props: {
  ...
  const t = useT()
  // Subscribe to keymap reloads: a live keybindings edit bumps this counter,
  // which re-renders the legend with the freshly-resolved tmux + in-pane
  // chords (the resolver's cache is cleared in the same reload). Read for the
  // subscription; the value itself isn't needed downstream.
  useKeymapVersion()
  ...
  const [prefixCap, setPrefixCap] = useState("Prefix")
  useEffect(() => {
    ...
    >
      ...
      {!folded
        ? hints.map((h, i) => {
            ...
            return (
              <box key={`${h.k}:${i}`} flexDirection="row" gap={1} justifyContent="space-between">
