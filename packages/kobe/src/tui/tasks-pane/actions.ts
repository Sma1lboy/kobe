/**
 * Tasks-pane action bodies — split out of `host.tsx` (which was over the
 * repo's 500-line file-size cap) into their own file, mirroring the
 * `settings-dialog/actions.ts` deps-bag template: each function takes an
 * explicit {@link TasksHostActionsContext} instead of closing over
 * `TasksShell`'s local signals. `host.tsx` keeps a thin 1-3 line wrapper
 * per function so call sites (JSX props, keybindings) are unchanged. Same
