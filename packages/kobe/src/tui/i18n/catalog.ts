 ...
 *
 * English (`en`) is the SOURCE OF TRUTH: it defines the `Messages` shape
 * every other locale must satisfy. `zh` (简体中文) must carry the exact same
 * key set — `scripts/check-i18n.mjs` (run in CI) fails the build on any
 * missing or extra key so the two never silently drift.
 *
 * Pure data, zero runtime deps — safe to import under node / vitest (the
 * reactive `t()` lives in `./index.ts`, which pulls solid-js). Values may
 * contain `{name}` placeholders; `t(key, params)` substitutes them.
 *
 * Migration is incremental: only strings already routed through `t()` need
 * a key here. We translate prose; literal config syntax (YAML keys, shell
 * command examples) is left in the calling code as-is — it isn't language.
 */

/** Keys mirror the on-screen structure of the Settings dialog. */
export const en = {
  settings: {
    title: "Settings",
    esc: "esc",
...
/**
 * Every locale must structurally match the English source of truth. `en` is
 * authored WITHOUT `as const` so leaves widen to `string` — locales supply
 * their own text, not a copy of the English literal.
 */
