/**
 * Workspace-host task-action wiring — extracted from `host.tsx` (file-size
 * cap). Builds the `CreateTaskContext` the shared `lib/task-actions` flows
 * run on (the SAME flows the tmux Tasks pane uses — confirm copy,
 * DIRTY_WORKTREE force-delete branch, error handling all live there so the
 * hosts can't drift) and returns the host's action callbacks.
 *
 * Only this host's genuine divergences are wired here: dialog surfacing,
 * toast notifications, and selection. No `switchBeforeKill` (no tmux client
 * to yank), no `openCreateSurface` (the in-pane NewTaskDialog IS the
 * surface), no `reload` (this host is fully signal-driven).
 */
