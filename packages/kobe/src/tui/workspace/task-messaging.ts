...
Read the installed Kobe skill's "Cross-task messaging" section before sending. Use normal \`kobe api send\` with \`peer_task_id\` as the target. Every message must include \`reply_to_task_id: ${current.id}\` so the receiving Task knows where to reply.
