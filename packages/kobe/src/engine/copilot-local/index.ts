...
export class CopilotLocal implements AIEngine {
  ...
  async spawn(cwd: string, prompt: string, opts?: SpawnOpts): Promise<SessionHandle> {
    return this.start({ cwd, prompt, opts, sessionId: randomUUID() })
  }
  ...
  async resume(sessionId: string, prompt: string, opts?: SpawnOpts): Promise<SessionHandle> {
    ...
    return this.start({ cwd, prompt, opts, sessionId })
  }
  ...
  private async start(args: {
    ...
    void (async () => {
      try {
        for await (const ev of parseStreamJson(readLines(spawned.stdout))) {
          if (ev.type === "done" || ev.type === "error") terminalSeen = true
          queue.push(ev)
          this.notify(session)
          if (ev.type === "done" || ev.type === "error") this.registry.unregister(session.sessionId, spawned.proc)
        }
      } catch (err) {
        queue.push({
          type: "error",
