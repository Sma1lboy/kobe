...
import { engineSpec, ensureTaskSession } from "../server/session.ts"
import type { RpcLink } from "../server/daemon-link.ts"
...
describe("web session launch init", () => {
  it("lets canonical tmux sessions receive the repo init first prompt", async () => {
    ...
    await ensureTaskSession(link(), "task-1")
  ...
  it("keeps web PTY engine specs from duplicating the repo init prompt", async () => {
    ...
    const spec = await engineSpec(link(), "task-2")
