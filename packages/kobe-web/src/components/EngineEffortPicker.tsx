...
/** `/api/engines` carries effortLevels per engine; the shared EngineOption type
 *  doesn't surface it, so read it off the runtime object here without widening
 *  the shared lib. */
function effortLevelsOf(engine: EngineOption | undefined): readonly string[] {
  const levels = (engine as { effortLevels?: readonly string[] } | undefined)
    ?.effortLevels
