...
export interface WorktreeChanges {
  ...
  /** Live branch from `git status --branch` (`## …` header), when present. */
...
export function parsePorcelain(text: string): WorktreeChanges {
  ...
  // `## ` header is present when the caller passed `--branch`; counting
  // still ignores it via parsePorcelainRows. Parse the live branch name:
  // `## <name>...<upstream>` → name before `...`; `## HEAD (no branch)` →
  // `HEAD`; `## No commits yet on <name>` → that name.
  for (const line of text.split("\n")) {
    if (!line.startsWith("## ")) continue
    const rest = line.slice(3)
    if (rest.startsWith("HEAD (no branch)")) branch = "HEAD"
    else if (rest.startsWith("No commits yet on "))
      branch = rest.slice("No commits yet on ".length)
    else {
      const dots = rest.indexOf("...")
      branch = dots >= 0 ? rest.slice(0, dots) : rest
    }
    break
  }
  ...
  return branch !== undefined ? { added, deleted, branch } : { added, deleted }
}
