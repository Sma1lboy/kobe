...
// `startAt` (capture seconds) lets other compositions embed a slice of the
// capture instead of always replaying from the boot.
export const QuickLookReplay: React.FC<{ speed?: number; startAt?: number }> = ({ speed = 1, startAt = 0 }) => {
  const frame = useCurrentFrame()
  const { fps } = useVideoConfig()
  const t = startAt + (frame / fps) * speed
